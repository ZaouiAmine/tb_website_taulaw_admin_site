...
import { useStates, useDeleteState } from "./states/queries";

export default function StatesPage() {
  const { t } = useTranslation();
  const navigate = useNavigate();
  const [open, setOpen] = useState(false);
  const [selectedRow, setSelectedRow] = useState<State | null>(null);

  // Use pagination hook
  const { setPaginate } = usePagination();

  const { data: response, isLoading, isPlaceholderData } = useStates();
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteStateMutation.mutateAsync(selectedRow.id);
      setOpen(false);
      setSelectedRow(null);
    } catch (error) {
  ...
  const columns: Column<State>[] = [
    {
      key: "nameEn",
      header: t("pages.states.columns.english"),
      width: "25%",
      cellClass: "py-6",
    },
    {
      key: "nameAr",
      header: t("pages.states.columns.arabic"),
      width: "25%",
    },
    {
      key: "nameFr",
      header: t("pages.states.columns.french"),
      width: "25%",
    },
  ];
  ...
  const actions: Action<State>[] = [
    {
      ...
      onClick: (row: State) => {
        ...
        navigate(`/admin/states/edit-state/${row.id}`);
      },
    },
    {
  ...
  if (isInitialLoading) {
    return <PageLoader message={t("pages.states.loading")} />;
  }

  return (
    <>
      <PageHeading
        heading={t("pages.states.title")}
        path={t("pages.states.path")}
      >
        <Link to={"/admin/states/add-state"}>
          <Button className="p-6 rounded-2xl text-lg">
            {t("pages.states.addTitle")}
          </Button>
        </Link>
      </PageHeading>
        ...
        <DialogContent className="sm:max-w-md py-8">
          <DialogHeader>
            ...
            <DialogDescription className="text-center mb-4 text-gray-700 text-lg">
              ...
              <b>{selectedRow?.nameEn || selectedRow?.nameAr}</b>.
            </DialogDescription>
          </DialogHeader>
