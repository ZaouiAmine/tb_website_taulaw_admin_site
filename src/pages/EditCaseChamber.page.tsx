...
import { useUpdateCaseChamber } from "./caseChambers/queries";
import { getCaseChambers } from "@/services/caseChambers";
import { useI18n } from "@/hooks/useI18n";
import { PageLoader } from "@/components/shared/CustomClipLoader";

export default function EditCaseChamberPage() {
  const { t } = useTranslation();
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  const { currentLanguage } = useI18n();
  const updateCaseChamberMutation = useUpdateCaseChamber();
  ...
  useEffect(() => {
    const fetchCaseChamber = async () => {
      if (!id) return;

      try {
        const response = await getCaseChambers({ page: 1, limit: 1000, search: "" });
        const caseChamber = response.data.flat().find(item => item.id === id);
        
        if (caseChamber) {
          form.reset({
            nameAr: caseChamber.nameAr,
            nameEn: caseChamber.nameEn,
            nameFr: caseChamber.nameFr,
          });
        } else {
          navigate("/dropdown-management/case-chamber");
        }
      } catch (error) {
        console.error("Error fetching case chamber:", error);
        navigate("/dropdown-management/case-chamber");
      }
    };
  ...
  function onSubmit(values: CaseChamberFormType) {
    ...
    updateCaseChamberMutation.mutate(
      { id, data: values },
      {
        onSuccess: () => {
          navigate("/dropdown-management/case-chamber");
  ...
  return (
    <div className="space-y-6">
      <PageHeading
        heading={t('pages.caseChamber.editTitle')}
          ...
          <div className="space-y-4 max-w-lg">
            ...
            <div className="grid grid-cols-2 gap-4 mt-10 w-full">
              <Button 
                className="p-6" 
                type="submit"
                disabled={updateCaseChamberMutation.isPending}
              >
                {updateCaseChamberMutation.isPending ? (
